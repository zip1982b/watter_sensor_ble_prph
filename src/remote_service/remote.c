...
static ssize_t on_write(struct bt_conn *conn, const struct bt_gatt_attr *attr, void *buf, uint16_t len, uint16_t offset, uint8_t flags);
...
{
	int8_t battery_level = get_battery_level();


	if (battery_level < 0) {
		LOG_ERR("error read adc battery_lvl = %d", battery_level);
	}
    else bt_bas_set_battery_level(battery_level);
}
